...
class Ticket(object):
    def __init__(self, **kw):
        self.__table = prettytable.PrettyTable(["车次", "出发站/到达站", "出发时间/到达时间", "历时", "商务座/特等座", "一等座", "二等座", "高级软卧" , "软卧", '硬卧', "硬座", "无座"])
    ...
    #定义函数完成K字开头的列车信息查询,kinds列表类型的参数用来存储需要查询的列车类别比如['K', "G"]:查询普快和高铁
    def check_train(self, kinds):
        for item in self.__data_list:
            ...
            if list1[9] == "Y":
                ...
                time_infor = str(list1[8]) + "\n" + "次日到达"
            common_list = [list1[1], from_st+"\n"+to_st,str(list1[6])+"\n"+str(list1[7]), time_infor]
            if list1[1].find('K') != -1 and ("K" in kinds):
                #将本次列车信息添加到表格中
                list = common_list+["--", "--", "--", "--",list1[21], list1[26], list1[27], list1[24]]
                self.__table.add_row(list)
            elif list1[1].find('D') != -1 and ("D" in kinds):
                # 将本次列车信息添加到表格中
                list = common_list + [list1[23], list1[29], list1[28], list1[31], list1[21], "--", "--", list1[24]]
                self.__table.add_row(list)
            elif list1[1].find('G') != -1 and ("G" in kinds):
                # 将本次列车信息添加到表格中
                if len(list1[23]) == 0:
                    td = "--"
                else:
                    td = list1[23]
                list = common_list + [td, list1[29], list1[28], "--","--", "--", "--", "--"]
                self.__table.add_row(list)
            elif list1[1].find('Z') != -1 and ("Z" in kinds):
                # 将本次列车信息添加到表格中
                list = common_list + ["--", "--", "--", list1[19],list1[21], list1[26], list1[27], list1[24]]
                self.__table.add_row(list)
            elif list1[1].find('T') != -1 and ("T" in kinds):
                # 将本次列车信息添加到表格中
                list = common_list + ["--", "--", "--", list1[19],list1[21], list1[26], list1[27], list1[24]]
                self.__table.add_row(list)
            else:
