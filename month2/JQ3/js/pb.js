class PB{
    //disX(disY):图片水平(竖直)方向间距,col_num列数
    constructor(disX, disY, col_num){
        ...
        //存储每一列具体的初始信息
        this.cols = this.get_col_coordition();
    }
    //定义函数获取每一列的横向坐标
    get_col_coordition(){
        let cols = [];
    ...
    this.x = disX;
        ...
        let timer = setInterval(function () {
            ...
            //元素和get(index)作用相同,只不过eq(index返回的是jQuery对象)
            let img = $('.pb img').eq(i);
            //获取当前新的列中最小的高度列
            let min = pb.get_min_col();
            //修改图片的left和top
